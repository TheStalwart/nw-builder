...
NwBuilder.prototype.zipAppFiles = function () {
    ...
    var numberOfPlatformsWithoutOverrides = 0;

    self._zips = {};

    this._forEachPlatform(function(name, platform) {
        var _needsZip = self.isPlatformNeedingZip(name, platform) || false;
        ...
        for (l18n in self.options.macPlistStrings) {
            ...
            if (typeof l18n === 'string') {
                if (typeof self.options.macPlistStrings[l18n] === 'string') {
                    var l18nFolder = path.resolve(platform.releasePath, self.options.appName+'.app', 'Contents', 'Resources', l18n+'.lproj');
                    // The folder may not exist
                    if (!fs.existsSync(l18nFolder)) {
                        fs.mkdirSync(l18nFolder);
                    }

                    var l18npath = path.resolve(l18nFolder, 'InfoPlist.strings');
                    allDone.push(Utils.copyFile(self.options.macPlistStrings[l18n], l18npath, self));
                } else {
...
NwBuilder.prototype.runApp = function () {
    ...
    self.emit('log', 'Launching App for platform ' + this.options.currentPlatform);
    return new Promise(function(resolve, reject) {
        var arguments = ['--enable-logging', self.options.files.replace(/\*[\/\*]*/,"")].concat(self.options.argv);
        var options = { stdio: 'inherit' };
        var nwProcess = self._nwProcess = spawn(executable, arguments, options);
